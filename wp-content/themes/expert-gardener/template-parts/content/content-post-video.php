...
<div id="post-<?php the_ID(); ?>" <?php post_class('blog-item'); ?>>
	<?php
		$expert_gardener_post_id = get_the_ID();
		$expert_gardener_post = get_post($expert_gardener_post_id);
		$expert_gardener_content = do_shortcode(apply_filters('the_content', $expert_gardener_post->post_content));
		$expert_gardener_embeds = get_media_embedded_in_content($expert_gardener_content);

		if (!empty($expert_gardener_embeds)) {
			foreach ($expert_gardener_embeds as $expert_gardener_embed) {
				$expert_gardener_embed = wp_kses($expert_gardener_embed, array(
					'iframe' => array(
						'src' => array(),
						'width' => array(),
						'height' => array(),
				...
				if (strpos($expert_gardener_embed, 'video') !== false || 
					strpos($expert_gardener_embed, 'youtube') !== false || 
					strpos($expert_gardener_embed, 'vimeo') !== false || 
					strpos($expert_gardener_embed, 'dailymotion') !== false || 
					strpos($expert_gardener_embed, 'vine') !== false || 
					strpos($expert_gardener_embed, 'wordpress.tv') !== false || 
					strpos($expert_gardener_embed, 'hulu') !== false) {
					?>
					<div class="custom-embedded-video">
						<div class="video-container">
							<?php echo $expert_gardener_embed; ?>
						</div>
